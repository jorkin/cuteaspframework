...
Cute.ui.dialog = Cute.Class.create({
	...
	_init: function() {
		...
		if (this.opt.mask) {
			if ($.browser.msie6) {
				this.mask = this.mask || $('<iframe class="Iframe_HideSelect" id="dialog_mask" scrolling="no" frameborder="0" hspace="0" frameSpacing="0" marginHeight="0" marginWidth="0" UNSELECTABLE="on"></iframe>').appendTo(document.body).attr("src", "/aboutblank.htm").hide();
			} else
				this.mask = this.mask || $('<div class="Iframe_HideSelect" id="dialog_mask"></div>').appendTo(document.body).hide();
		}
		var _html = [];
	...
	_regEvent: function() {
		...
		if (this.opt.drag) this.els.head.drag(window, _dialog, { x: 10, y: 10 });
		if (this.opt.blur) this.pannel.out("click", this.close.bind(this), true);
	},
	_show: function() {
		this.pannel.show();
		this.resize();
	...
	setPos: function(pos) {
		...
		if (pos) {
			...
			if (this.opt.parentEl) {
				pannelBox.css({ "top": "auto", "left": "auto", "bottom": "auto", "right": "auto" });
			} else {
	...
	close: function(time) {
		if (time && time.constructor == Number) {
			if (!$.browser.msie && this.opt.close.duration) {
				this.timer = function() {
					this.els.dialogBox.animate({ opacity: 0.1 }, function() {
	...
	ajax: function(title, options) {
		...
		this._optionsExtend(this.opt, options);
		if (this.opt.action) {
			Cute.api.get(this.opt.action, this.opt.params || {}, function(html) {
				this._init();
				this.setContent(html);
				this.show();
			} .bind(this), false, true, options.ajaxoptions || {});
		}
		return this;
	},
	...
	suggest: function(info, options) {
		...
		this.els.dialogBox.stop(true, true).css({
			...
			this.els.dialogBox.delay(1200).animate({
				"opacity": 0.1,
				"top": this.els.dialogBox.position().top + 30
			}, 1200, function() {
				this.close();
