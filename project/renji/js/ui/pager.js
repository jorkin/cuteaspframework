...
Cute.ui.pager = Cute.Class.create({
	initialize: function(obj, options) {
		...
		if (!this.opt.totalcount && this.opt.currpagetotal) {
			this.opt.type = "text";
		}
		return this;
	},
	_init: function() {
		...
		if (_this.opt.pagecount > 1) {
			if (_this.opt.type == "text") {
				if (_this.opt.pageindex > 1) {
					...
					_html.push('<a class="p_prev" rel="' + _this._getPageIndex(_this.opt.pageindex - 1) + '" href="' + _this._getUrl(_this.opt.pageindex - 1) + '">上一页</a>\n');
				}
				if (_this.opt.pageindex < _this.opt.pagecount) {
					if (_this.opt.currpagetotal == -1 || _this.opt.currpagetotal >= _this.opt.pagesize) {
						_html.push('<a class="p_next" rel="' + _this._getPageIndex(_this.opt.pageindex + 1) + '" href="' + _this._getUrl(_this.opt.pageindex + 1) + '">下一页</a>\n');
					}
			...
			if (_this.opt.type == "numeric") {
				if (_this.opt.pageindex > 1) {	//第一页
					_html.push('<a class="p_prev" rel="' + _this._getPageIndex(_this.opt.pageindex - 1) + '" href="' + _this._getUrl(_this.opt.pageindex - 1) + '">上页</a>\n');
				}
				...
				$.each(_page, function(i, item) {
					if (this.opt.pageindex == item) {
						...
						//                        Cute.log(item, "item");
						//                        Cute.log(_ellipsis[1], "_ellipsis[1]");
						...
						_html.push('<a class="p_start" rel="' + this._getPageIndex(item) + '" href="' + this._getUrl(item) + '" title="' + title + '">' + item + '</a>\n');
				...
				if (_this.opt.pageindex < _this.opt.pagecount) {
					_html.push('<a class="p_next" rel="' + _this._getPageIndex(_this.opt.pageindex + 1) + '" href="' + _this._getUrl(_this.opt.pageindex + 1) + '">下页</a>\n');
				}
			}
			if (_this.opt.skip) {
			...
			Cute.params.set({ "page": _this.opt.pageindex });
		}
		if (_this.opt.pagecount > 1 || _this.opt.total) {
			_html.push('</div>');
		}
		...
		if (_this.opt.ajaxload) {
			$("a[class^=p_]", _this.element).click(function() {
				_this.element.html("数据加载中...");
				_this.opt.pageindex = parseInt($(this).attr("rel"));
				_this.opt.ajaxcallback(_this.element.selector, _this.opt.pageindex);
				_this._init();
				return false;
			});
		}
