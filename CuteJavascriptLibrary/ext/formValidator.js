	...
	$.formValidator = {
		...
		pageIsValid: function(validatorgroup) {
			...
			if (isvalid) {
				...
				if (thefirstid != "" && initConfig.errorfocus) { $("#" + thefirstid).trigger("focus.form"); }
			}
	...
	$.fn.formValidator = function(cs) {
		...
		return this.each(function(e) {
			...
			if (srcTag == "input" || srcTag == "textarea") {
				...
				jqobj.bind("focus.form", function() {
					if (!initConfig.alertmessage) {
						//保存原来的状态
						var tipjq = $("#" + tip);
						this.lastshowclass = tipjq.attr("class");
						this.lastshowmsg = tipjq.html();
						if (setting.onfocus) $.formValidator.setTipState(this, "onFocus", setting.onfocus);
					}
						...
						if (returnObj.isvalid) {
							...
							if (auto && !initConfig.alertmessage) {
								...
								if (initConfig.forcevalid || setting.forcevalid) {
									alert(showmsg); this.trigger("focus.form");
								}
							}
						}
					}
